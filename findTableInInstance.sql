--Filename: findTableWithinInstance
...
declare @tblNameSample varchar(250)
set @tblNameSample = '%profile%' --Note that any wild cards must be in this string. 
...
order by d.name
--Loop through the list of databases to see if any of them contains a column matching @tblNameSample 
open dbs
...
begin
	...
	set @sqlStmt = @sqlStmt +'select '''+@db+''',s.name, t.name '
	set @sqlStmt = @sqlStmt +'from sys.tables AS t '
	set @sqlStmt = @sqlStmt +'inner join sys.schemas s on t.schema_id = s.schema_id '
	set @sqlStmt = @sqlStmt +'where t.[name] like '''+@tblNameSample+''' ' 
	begin try
		EXEC (@sqlStmt)
	end try
	begin catch
